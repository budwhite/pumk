class RidesController < ApplicationController
  ...
  def update
    ...
    all_addresses = current_user.addresses + Address.where(user_id: 0).all 
    orig_id = all_addresses.detect{|x| x.name == params[:ride][:origin]}.id
    dest_id = all_addresses.detect{|x| x.name == params[:ride][:destination]}.id
    ...
    if ride.update_attributes(
      created_as: created_as,
      seats_total: params[:ride][:seats_total],
      gas_money: params[:ride][:gas_money],
      ride_type: params[:ride][:ride_type],
      origin_address_id: orig_id,
      destination_address_id: dest_id,
      schedule: schedule
    )
      if created_as == 'rider'
        rds = ride.riderships.where(user_id: current_user.id).first 
        if rds.nil?
          ride.riderships.create!(
            user_id: current_user.id,
            ride_id: ride.id,
          ...
          rds.update_attributes(child_id: for_which_child_id)
        end
      end
